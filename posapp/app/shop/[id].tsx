...
const OrderIdPage = () => {
  ...
  const [orderData, setOrderData] = useState<any>({}); // State for order data
...
// Render a product item
  ...
  const renderQueueItem = ({ item }: { item: any }) => (
    ...
    // Add product to cart
  const addToCartByQueue = async (queue: any) => {
    setLoading
    try {
      const response = await axios.get(`${apiUrl}/order/${queue}`); // Fetch queue from the API
      setOrderData(response.data.order); // Set queue state
      response.data.order.products.forEach((product: any) => {
        setCart((prevCart) => {
  ...
  const handleCheckoutSubmit = async () => {
    ...
    try {
      ...
      const response = await axios.put(`${apiUrl}/order/update-to-completed/${id}`, order);

      if (response.status === 200) {
        alert('Order successfully updated and marked as completed!');
        setCart([]); // Clear the cart after successful update
